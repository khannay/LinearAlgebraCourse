...
\begin{lstlisting}
pop=zeros(15,1);
for i=1:15; pop(i)=sum(L^(i-1)*x0); end;
bar(0:14, pop)
\end{lstlisting}
...
\begin{lstlisting}
pop=zeros(20,1);
for i=1:20; pop(i)=sum(L2^(i-1)*x0); end;
bar(0:19, pop)
\end{lstlisting}
...
You will need to download the matlab files (and unzip them) for the project and change the directory in matlab/octave to the folder you extracted the zip file to. You will now be able to use the function image\_compression
...
Here is an example of doing this for 10 components:
\begin{lstlisting}
image_compression('gilbert_strang.jpg', 10);
\end{lstlisting}
...
\begin{lstlisting}
[R1,C]=image_compression('gilbert_strang.jpg', 10);
bar(svd(R1)) %this will make a bar plot of the singular values
\end{lstlisting}
